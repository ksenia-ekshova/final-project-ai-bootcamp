...
const startGameSession = async (gameSettings, userPrompt, isFirstTurn = false) => {
    ...
    if (isFirstTurn) {
        assistant = await openai.beta.assistants.create({
            ...
            model: gameSettings.gptVersion === 'GPT-4' ? "gpt-4-turbo" : "gpt-3.5-turbo",
        });
        ...
        if (!thread || !assistant) { return "rofl" }
        const message = await openai.beta.threads.messages.create(
            thread.id,
            {
                role: "assistant",
